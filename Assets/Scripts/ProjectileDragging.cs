//this will create and affect our line renderer as well as the astroid itself
...
public class ProjectileDragging : MonoBehaviour {
	...
	public LineRenderer catapultLineFront; //references to the two lines we are effecting
	public LineRenderer catapultLineBack; //these will be dragged in from the editor
	...
	void Awake(){
		...
		catapult = spring.connectedBody.transform;//we know the spring, and the spring is connected to the catapult and we get the transform of the catapult
	...
	void Start () {
		...
		rayToMouse = new Ray(catapult.position, Vector3.zero); //this ray will start at our catapult position
	...
	void Update () {
		if (clickedOn){ //if we are clicked on the object
		...
		if(spring != null){ //if we still have a spring, we haven't launched it yet
			...
			if(!GetComponent<Rigidbody2D>().isKinematic && prevVelocity.sqrMagnitude > GetComponent<Rigidbody2D>().velocity.sqrMagnitude){
				...
				GetComponent<Rigidbody2D>().velocity = prevVelocity; //assign last frame's velocity to be the current velocity so it doesn't lose speed
			}

			if(!clickedOn){ //if we have released the button but the rock hasn't crossed the point yet
				prevVelocity = GetComponent<Rigidbody2D>().velocity; //we set the prevVelocity to be the current velocity for the future
			}
		...
		} else{ //if the spring has been destroyed, when we launched
	...
	void OnMouseUp(){
		...
		GetComponent<Rigidbody2D>().isKinematic = false; //the rb becomes a dynamic rb to use physics
		clickedOn = false; //we are no longer dragging
		anim.SetTrigger("released");
	}

	void Dragging(){ //is called each frame we are dragging the rock
		Vector3 mouseWorldPoint = Camera.main.ScreenToWorldPoint (Input.mousePosition); //gets the position of the mouse
